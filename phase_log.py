...
def init_data():
  sql_text2 = 'TRUNCATE TABLE phase_b_log'
...
if __name__ == '__main__':
  ...
  cursor1 = cnx1.cursor()
  print funx.get_time(), '源数据库已连接'

  cnx2 = mysql.connector.Connect(**cnx2_cfg)
  cursor2 = cnx2.cursor()
  funx.get_time()
  print funx.get_time(), '目标数据库已连接'
  ...
  print funx.get_time(), '源数据数:', cursor1.rowcount, '开始添加数据到目标数据表'
  ...
  for rowData in data1:
    ...
    if cursor2.rowcount != 1:
      print funx.get_time(), '编号', pid, '数据出错！'
